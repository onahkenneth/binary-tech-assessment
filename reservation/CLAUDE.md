---
description: Use Bun instead of Node.js, npm, pnpm, or vite. Plus project-specific guidelines for the Reservation Service.
...
- **Service Layer**: Business logic in `src/services/reservation.ts`
- **Data Access Layer**: Database operations in `src/model/ReservationRepository.ts`
...
### GET /reservation
Retrieve all reservations with their status
...
test("should get all reservation", async () => {
  const service = ReservationService.getInstance();
